...
query_id = 5753743
table_name = "Weekly_Prediction_Market_Notional_Volume"
...
SUM(value*price/POW(10, decimals)) as "Notional USD Volume"
FROM trades t
...
SELECT DATE_TRUNC('week', evt_block_time) as "Week",
CASE
WHEN CAST(makerAssetId AS VARCHAR) = '0' THEN CAST(takerAmountFilled/2 AS DOUBLE) / 1e6
WHEN CAST(takerAssetId AS VARCHAR) = '0' THEN CAST(makerAmountFilled/2 AS DOUBLE) / 1e6
END AS "Volume"
...
kalshi as (
SELECT DATE_TRUNC('week', CAST(date as DATE)) as "Week", SUM(value) as "Notional USD Volume" FROM dune.datadashboardsapi.dataset_kalshi_daily_metrics
WHERE "Type" = 'Volume'
GROUP BY 1
),
...
SELECT "Week", SUM("Volume") as "Notional Volume" FROM (
SELECT DATE_TRUNC('week', block_time) as "Week",
CASE
WHEN CAST(makerAssetId AS VARCHAR) = '0' THEN CAST(takerAmountFilled/2 AS DOUBLE) / 1e18
WHEN CAST(takerAssetId AS VARCHAR) = '0' THEN CAST(makerAmountFilled/2 AS DOUBLE) / 1e18
END AS "Volume"
FROM orderfills
)
GROUP BY 1
...
UNION ALL
SELECT *, 'Kalshi' as platform FROM kalshi
UNION ALL
